...
class SpatialTemporalTransformer(nn.Module):
    ...
    def __init__(self, n_input, t, n_grid, n_spatial_layer=4, n_temporal_layer=3, emb_dim=64, n_head=4, drop_rate=0.1):
        ...
        # 2. 共享参数
        self.spatial_share = SpatialTransformer(n_input, n_grid, n_spatial_layer, emb_dim, n_head, drop_rate)
    ...
    def forward(self, x):
        ...
        for i in range(x.shape[0]):
            o = self.spatial[i](x[i])  # [n_car, n_grid, input] 不共享
            # o = self.spatial_share(x[i])  # 共享
            o = torch.reshape(o, (o.shape[0], -1))
            o = self.process_s(o)
            temporal_input[i] = o

        # # 1. 将每个输入的中间位置取出来（目标车辆的位置）
        # for k, _ in enumerate(spatial_output):
        #     r = torch.permute(spatial_output[k], (1, 0, 2))
        #     index = int(self.n_grid / 2)  # 21 / 2 = 10
        #     temporal_input[k] = r[index]

        # 2. 将每一个空间transformer的输出累加在一起
        # for k, _ in enumerate(spatial_output):
        #     r = spatial_output[k]  # [n_car, n_grid, input]
        #     temporal_input[k] = torch.sum(r, dim=1)  # [n_car, input]
...
class EnvironmentOperateModule(nn.Module):
    def __init__(self, n_input, t, n_grid, n_spatial_layer=3, n_temporal_layer=3, emb_dim=64, n_head=4, drop_rate=0.1):
        ...
        # self.spatial = nn.ModuleList([
        #     SpatialTransformer(n_input, n_grid, n_layer, emb_dim, n_head, drop_rate) for i in range(t)]
        # )
    ...
    def forward(self, x):
        # x = torch.permute(x, (1, 0, 2, 3))  # [t, n_car, n_grid, input_shape]
        # spatial_output = []
        # res = torch.zeros((self.t_windows, x.shape[1], self.emb_dim))  # [t, n_car, embed_dim]
        # for i in range(x.shape[0]):
        #     o = self.spatial[i](x[i])  # [n_car, n_grid, emb_dim]
        #     spatial_output.append(o)
        #
        # for k, _ in enumerate(spatial_output):
        #     r = torch.permute(spatial_output[k], (1, 0, 2))
        #     index = int(self.n_grid / 2)  # 21 / 2 = 10
        ...
        # res = torch.reshape(res, (res.shape[0], -1))  # [n_car, t * emb_dim]
        # res = res.cuda()
        o = relu(self.fc_i(st))  # [n_car, embed_dim]
        for v in self.fc_h:
            o = relu(v(o))
        o = self.fc_o(o)  # 加一个residual结构
        return o
...
class Generator(nn.Module):
    ...
    def forward(self, x):
        ...
        s = self.process_s(s)
        # 这里也不只用本车所在的embedding，而是将环境所有embedding累加在一起
        # en_embed = torch.sum(s, dim=1)

        # 尝试不用多秒的时空transformer，只用空间transformer
        fc_input = torch.cat(
            (
                s,
                st,
...
class Generator_1(nn.Module):
    ...
    def __init__(self, n_input, output_dim, t, n_grid, target='reg', n_spatial_layer=3, n_temporal_layer=3, emb_dim=32,
        ...
        self.s = SpatialTransformer(n_input, n_grid, n_temporal_layer, emb_dim, n_head, drop_rate)
        # fully connect layer
        self.process_st = FullyConnectLayer(emb_dim * n_grid, emb_dim)
        self.fc = FullyConnectLayer(emb_dim * 2, 300)  # 需要将输出变成两种，一种是softmax，一种是
        self.embed = PositionEmbedding(emb_dim, emb_dim, input_dim=n_input, is_position=False)
        self.n_grid = n_grid
        self.c = nn.Sequential(
            nn.Linear(300, 300),
    ...
    def forward(self, x):
        # 输入维度 [n_car, n_grid, embed_dim]
        # 为了保证初始结果的差异,需要使用一个非常长的residual结构,将输入中的当前车辆的信息,经过embedding后加到时空encoder的输出中
        now_x = x  # [n_car, n_grid, n_input]
        now_aim_x = torch.permute(now_x, (1, 0, 2))[self.n_grid // 2]  # [n_car, n_input]
        now_aim_x = now_aim_x.cuda()
        now_embed = self.embed(now_aim_x)
        s = self.s(x)  # [n_car, n_grid, embed_dim]
        # en_embed = torch.sum(s, dim=1)
        s = torch.reshape(s, (s.shape[0], -1))
        # print(s.shape)
        s = self.process_st(s)
        # print(s.shape)
        # 尝试不用多秒的时空nsformer，只用空间transformer
        fc_input = torch.cat(
            (
                # en_embed,
                s,
        ...
        # fc_input = torch.permute(s, (1, 0, 2))[self.n_grid // 2] + now_embed
        o = self.fc(fc_input)  # [n_car, 5]
        # 将FC layer的输出分别送入两个分支，一个是分类器，一个是回归器
        if self.target == 'reg':
            o = self.o(o)
...
class Discriminator_1(nn.Module):
    def __init__(self, n_input, r_input, t, n_grid, n_spatial_layer=3, n_temporal_layer=3, emb_dim=64, n_head=4,
        ...
        super().__init__()
        self.result_operator = ResultOperateModule(r_input, emb_dim)
        # self.environment_operator = EnvironmentOperateModule(n_input, t, n_grid, n_spatial_layer, n_temporal_layer,
        #                                                      emb_dim, n_head, drop_rate)
        self.s = SpatialTransformer(n_input, n_grid, n_spatial_layer, emb_dim, n_head, drop_rate)
        self.embed = PositionEmbedding(emb_dim, emb_dim, input_dim=n_input, is_position=False)
        self.n_grid = n_grid
        self.fc = nn.Sequential(
            nn.Linear(emb_dim * 2, 300),
    ...
    def forward(self, x, y):
        ...
        # fc_input = torch.permute(s, (1, 0, 2))[self.n_grid // 2] + now_embed
        # o = self.fc(fc_input)  # [n_car, 5]
        e = torch.permute(s, (1, 0, 2))[self.n_grid // 2] + now_embed
        # print(y.shape)
        r = self.result_operator(y)  # [n_car, embed_dim]
        # e = self.environment_operator(fc_input)  # [n_car, embed_dim]
